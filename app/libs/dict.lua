...
local sharedData = ngx.shared.ngx_data
...
- return  mix
--]]
function _M:get_or_load(key, cb, timeout)
    local value, err
    ...
    if not value then
        ...
        if value then
            local ok, err = _M:set_json(key, value, timeout)
            if not ok then
...
-- 保存到存储系统并更新本地缓存
function _M:save_and_update(key, value, cb, timeout)
...
-- 从存储获取并更新缓存
function _M:load_and_set(key, cb, timeout)
    ...
    if err or not value then
        ...
        local ok, errr = _M:set(key, value, timeout)
...
function _M:set_json(key, value, timeout)
    if value then
        value = cjson.encode(value)
    end
    return _M:set(key, value, timeout)
end

function _M:set(key, value, timeout)
    return sharedData:set(key, value, timeout)
end
